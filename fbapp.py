...
from status_fetcher import AttrDict
...
class BaseHandler(webapp.RequestHandler):
    ...
    def render(self, name, **data):
        ...
        self.response.out.write(template.render(
            os.path.join(
                os.path.dirname(__file__), 'templates/app', name + '.html'),
            data))
...
class RecentAnaHandler(BaseHandler):
	def get(self):
		if self.user:
			analyzer = StatusAnalyzer()
			user = self.user
			access_token = user.access_token
			results = urlfetch.fetch("https://graph.facebook.com/"+user.user_id+"/feed?access_token="+access_token+"&limit=550",method=urlfetch.GET,deadline=10)
			...
			self.render(u'analyze',user=user,stats=stats,recent=active_friends,inactive=inactive_friends)
		else:
			self.render(u'welcome')
			
class UserAnaHandler(BaseHandler):
